...
class TestCourseList:
    ...
    def test_draft_course_not_in_student_list(self, auth_client, teacher_user, category):
        Course.objects.create(
            title="Cours Brouillon",
            slug="cours-brouillon",
...
class TestCourseDetail:
    def test_get_course_detail(self, auth_client, course):
        url = reverse("courses:courses-detail", kwargs={"pk": course.id})
        response = auth_client.get(url)
        assert response.status_code == 200
        assert response.data["title"] == course.title

    def test_course_not_found(self, auth_client):
        url = reverse("courses:courses-detail", kwargs={"pk": 99999})
        response = auth_client.get(url)
...
class TestEnrollment:
    def test_student_can_enroll(self, auth_client, course, student_user):
        url = reverse("courses:courses-enroll", kwargs={"pk": course.id})
...
class TestCourseCreation:
    def test_teacher_can_create_course(self, teacher_client, category):
        ...
        response = teacher_client.post(url, payload)
    ...
    def test_student_cannot_create_course(self, auth_client, category):
        ...
        payload = {
            ...
            "slug": "cours-interdit",
            "description": "Un étudiant ne peut pas créer",
            "category": category.id,
        }
