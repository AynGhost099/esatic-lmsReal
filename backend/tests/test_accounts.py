...
class TestUserRegistration:
    def test_register_student(self, api_client):
        ...
        user = User.objects.get(email="nouveau@esatic.ci")
...
class TestAuthentication:
    def test_login_success(self, api_client, student_user):
        ...
        response = api_client.post(url, {"email": student_user.email, "password": "StudentPass123!"})
    ...
    def test_login_unknown_email(self, api_client):
        url = reverse("accounts:login")
        response = api_client.post(url, {"email": "inconnu@esatic.ci", "password": "SomePass"})
        assert response.status_code == 401

    def test_token_refresh(self, api_client, student_user):
        login_url = reverse("accounts:login")
        tokens = api_client.post(login_url, {"email": student_user.email, "password": "StudentPass123!"}).data
...
class TestUserProfile:
    ...
    def test_update_own_profile(self, auth_client):
        url = reverse("accounts:me")
        response = auth_client.patch(url, {"bio": "Étudiant en informatique"})
        assert response.status_code == 200
